...
Response format (required): You MUST output your reasoning first inside <thinking>...</thinking>, then exactly one JSON object.
- In <thinking>: (1) What do you see in the screenshot? (2) What does the user want? (3) Which UI element or action will you use and why?
- Then on the next line: exactly one JSON object, no other text.
Example:
<thinking>
...
def _run_router_standalone(task: str, image_path: str, ui_elements: list, use_local: bool, api_key: str, model: str, learning_context: str):
    """
    Run router in a separate process (no GUI). Returns (action, payload, raw_response).
    Used so local model inference doesn't block the main process.
    """
    try:
        ...
        if use_local:
            from ollama_vl import call_ollama
            raw_response = call_ollama(system, user_text, img, conversation_messages=None, max_tokens=4096)
        else:
            user_content = [
                {"type": "text", "text": user_text},
                {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{b64}"}},
            ]
            raw_response = _call_qwen(api_key, model, system, user_content, max_tokens=4096)
        text = raw_response
        ...
        return ("comment", f"API error: {e}", str(e))
...
class AgentGUI:
    ...
    def __init__(self, root):
        ...
        # If using Ollama, check it's available
        if self.use_local_var.get():
            def _check_ollama():
                try:
                    from ollama_vl import is_ollama_ready, OLLAMA_VISION_MODEL
                    if is_ollama_ready():
                        self.root.after(0, lambda: self.log(f"Ollama ({OLLAMA_VISION_MODEL}) ready.", "info"))
                    else:
                        self.root.after(0, lambda: self.log("Ollama not running or model missing. Start Ollama and run: ollama pull qwen3-vl:30b", "warning"))
                except Exception as e:
                    self.root.after(0, lambda: self.log(f"Ollama check failed: {e}. Install: pip install ollama", "error"))
    ...
    def _open_settings(self):
        ...
        use_local_cb = tk.Checkbutton(
            win, text="Use local model via Ollama (free, no API key) – run: ollama pull qwen3-vl:30b",
    ...
    # ==================================================================
    def _run_router_ollama(self, task, screenshot, ui_elements=None):
        """Run router using Ollama in this thread (no subprocess). Returns (action, payload, raw_response)."""
    ...
    def _ask_claude(self, task, screenshot, ui_elements=None):
        """Send screenshot + task to vision model (API path only). Returns (action, payload, raw_response)."""
        use_local = self.use_local_var.get()
        if not use_local:
            api_key = self.claude_api_key_var.get().strip()
            if not api_key:
                return ("comment", "Enable 'Use local model (Ollama)' or set DashScope API key in Settings.", "")
    ...
    def _ask_claude_next_action(self, task, screenshot, action_history: list, conversation_messages: list = None, thought_history: list = None, ui_elements=None):
        ...
        if not use_local and not self.claude_api_key_var.get().strip():
            return ("comment", "Enable 'Use local model (Ollama)' or set DashScope API key.", conversation_messages or [], thought_history or [])
...
You MUST use two phases in your response:

PHASE 1 - THINKING (required): First output a <thinking>...</thinking> block. Inside it, state: (1) What you see in the screenshot and what the user's task is. (2) What you have already done. (3) Which UI element or action you will use next and why. Be specific so the next step is clear.

PHASE 2 - OUTPUT (required): Right after </thinking>, on the next line output exactly one JSON object. Valid JSON only, no extra text.
...
{"action": "TYPE_TEXT", "parameters": {"text": "notepad"}}"""
        ...
        try:
            if use_local:
                ...
                text = call_ollama(system, user_text, img, conversation_messages=local_history, max_tokens=16384)
    ...
    def _start(self):
        ...
        if self.use_local_var.get() or self.claude_api_key_var.get().strip():
            ...
            if is_chess_task(task):
                ...
                self.log("Enable 'Use local model (Ollama)' or add DashScope API key in Settings.", "warning")
                self._running = False
    ...
    def _start_with_claude(self, task):
        """Take screenshot, run router (in process if local model), then _on_router_done on main thread."""
        try:
            ...
            use_local = self.use_local_var.get()
            if use_local:
                self.root.after(0, lambda: self.status_label.config(text="Calling Ollama...", fg="#FF9800"))
                self.log("Calling Ollama...", "dim")
                try:
                    action, payload, raw_response = self._run_router_ollama(task, screenshot, ui_elements)
                except Exception as e:
                    self.log(f"Router error: {e}", "error")
                    action, payload, raw_response = "comment", f"Error: {e}", ""
            else:
                self.root.after(0, lambda: self.status_label.config(text="Calling model...", fg="#FF9800"))
                self.log("Calling DashScope API...", "dim")
                action, payload, raw_response = self._ask_claude(task, screenshot, ui_elements)

            # Schedule all GUI updates on main thread
    ...
    def _on_router_done(self, task, action, payload, raw_response, screenshot):
        ...
        self.action_label.config(text=display)
